...
- `POST /api/provider-catalog/:providerId/jobs` create a provider worker dry-run job
...
## Production Upgrade
Replace the in-memory store with BullMQ + Redis and connect workers to FFmpeg or the provider worker manifests from the provider catalog.
